...
**This content has moved to a [new location](https://datascience.ibm.com/docs/content/analyze-data/ml_dlaas_working_with_new_models.html). Check there for the most up-to-date information.**
...
* `model_definition.execution`: This field provides information concerning the command to launch the training.
    ...
    - `model_definition.compute_configuration.name`: This field specifies the resources that will be allocated for training and should be one of the following values - `k80` (1 k80 GPU), `k80x2` (2 k80 GPUs), `k80x4` (4 k80 GPUs), `p100` (1 p100 GPU), `p100x2` (2 p100 GPUs), `v100` (1 v100 GPU), `v100x2` (2 v100 GPUs).
...
# Generate a sample training run manifest file.

Sample manifest file template can be generated by using the `bx ml generate-manifest` command: `bx ml generate-manifest training-runs`
Edit the generated manifest with appropriate values for access_key_id, secret_access_key and bucket fields.
```
bx ml generate-manifest training-runs
```
{: codeblock}
